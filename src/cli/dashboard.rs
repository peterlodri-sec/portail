...
impl<T: Copy + Default> Ring<T> {
    fn new() -> Self {
        Self { buf: [T::default(); RING_CAP], head: 0, len: 0 }
    }
    ...
    fn last(&self) -> T {
        if self.len == 0 { T::default() } else { self.buf[(self.head + RING_CAP - 1) % RING_CAP] }
    }
}
...
enum Tab { Network, Events, Hooks, Cache, Health }

impl Tab {
    const ALL: &'static [Tab] = &[Tab::Network, Tab::Events, Tab::Hooks, Tab::Cache, Tab::Health];
...
impl Dashboard {
    ...
    pub fn push_net_sample(&mut self, total_reqs: u64, total_bytes_in: u64, total_bytes_out: u64, total_errors: u64, latency_us: u64) {
    ...
    pub fn set_events(&mut self, events: Vec<EventEntry>) { self.events = events; }
    pub fn set_hooks(&mut self, hooks: Vec<HookEntry>) { self.hooks = hooks; }
    pub fn set_cache(&mut self, cache: CacheStats) { self.cache = cache; }
    pub fn set_health(&mut self, health: HealthStatus) { self.health = health; }
    ...
    pub fn render_text(&self) -> String {
        ...
        o.push_str(&format!("network: {} req/s (last), {} samples\n",
            self.net_reqs.last(), self.net_reqs.len));
        o
    }
    ...
    fn draw_network(&self, f: &mut Frame, area: Rect) {
        ...
        self.draw_sparkline(f, cols[0], "Requests/s", self.net_reqs.as_slice(), Color::Green);
        self.draw_sparkline(f, cols[1], "Bytes In/s", self.net_bytes_in.as_slice(), Color::Cyan);
        self.draw_sparkline(f, cols[2], "Bytes Out/s", self.net_bytes_out.as_slice(), Color::Blue);
        self.draw_sparkline(f, cols[3], "Latency μs", self.net_latency.as_slice(), Color::Yellow);
    }
    ...
    fn draw_net_detail(&self, f: &mut Frame, area: Rect) {
        ...
        let counters = Paragraph::new(Line::from(vec![
            Span::styled(format!("  req/s: {}  ", reqs), Style::default().fg(Color::Green)),
            Span::styled(format!("in: {}  ", human_bytes(bi)), Style::default().fg(Color::Cyan)),
            Span::styled(format!("out: {}  ", human_bytes(bo)), Style::default().fg(Color::Blue)),
            Span::styled(format!("errs: {}  ", errs), Style::default().fg(Color::Red)),
            Span::styled(format!("lat: {}μs", lat), Style::default().fg(Color::Yellow)),
        ]))
    ...
    fn draw_events(&self, f: &mut Frame, area: Rect) {
        let items: Vec<ListItem> = self.events.iter().rev().map(|e| {
        ...
        f.render_widget(
            List::new(items).block(Block::default().borders(Borders::ALL).title(" Agent Events ")),
            area,
        );
    }

    fn draw_hooks(&self, f: &mut Frame, area: Rect) {
        let items: Vec<ListItem> = self.hooks.iter().map(|h| {
            let (sym, color) = if h.enabled { ("✓", Color::Green) } else { ("✗", Color::Red) };
            ListItem::new(Line::from(vec![
    ...
    fn draw_cache(&self, f: &mut Frame, area: Rect) {
        ...
        let rate = if total > 0 { self.cache.hits as f64 / total as f64 } else { 0.0 };
        ...
        f.render_widget(
            ...
            .block(Block::default().borders(Borders::ALL).title(" Cache Stats ")),
            cols[1],
        );
    }
